    ...
    return (
        <>
            <div className='flex justify-end p-5 space-x-5 items-center'>
                ...
                <CgMenuGridO className='text-4xl bg-transparent hover:bg-slate-200 rounded-full p-2 cursor-pointer' onClick={() => setToggle(!Toggle)} />
                <Avtar url={profile}/>
                {/* <button className='bg-blue-600  text-white rounded-md py-1 px-3 hover:brightness-105'>Sign in</button> */}
            </div>
            {Toggle && <Modal className='cursor-pointer'/>}
        </>
    )
}
